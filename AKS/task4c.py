...
original_image = cv2.imread('imgg/Characters Test Pattern 688x688.tif', 0)
...
for u in range(row):
        for v in range(column):            
            ...
            ghf[u,v] = 1 - np.exp(-((D**2) / (2 * D0**2)))


foutput_img = fimg * ghf
# foutput_img = np.fft.ifftshift(foutput_img)
